...
// InitAuth reads the GitHub OAuth app and the signing key from the environment.
func InitAuth() {
	githubOauthConfig = &oauth2.Config{
	...
	if githubOauthConfig.ClientID == "" || githubOauthConfig.ClientSecret == "" {
		logrus.Warn("GitHub OAuth credentials are not set: sign-in is off and the instance is open.")
	}
...
// newState starts a sign-in: the value goes to GitHub and into a cookie, and
// the callback accepts only a code that comes back with the same value.
// Without the check anyone could make a browser sign in to an account of their
// choosing by sending it a callback link.
func newState(w http.ResponseWriter, r *http.Request) (string, error) {
	b := make([]byte, 16)
...
// stateMatches checks the callback's state against the cookie and clears it.
func stateMatches(w http.ResponseWriter, r *http.Request) bool {
	cookie, err := r.Cookie(stateCookieName)
	if err != nil || cookie.Value == "" {
		return false
	}
	http.SetCookie(w, &http.Cookie{Name: stateCookieName, Value: "", Path: "/auth/", MaxAge: -1})
	return subtle.ConstantTimeCompare([]byte(cookie.Value), []byte(r.FormValue("state"))) == 1
}

// HandleLogin sends the browser to GitHub.
...
// HandleCallback finishes the sign-in GitHub sends back.
func HandleCallback(w http.ResponseWriter, r *http.Request) {
	...
	if !stateMatches(w, r) {
		logrus.Warn("rejected sign-in callback: state does not match")
		http.Redirect(w, r, "/auth/signed-out", http.StatusTemporaryRedirect)
		return
	}

	token, err := githubOauthConfig.Exchange(r.Context(), r.FormValue("code"))
	if err != nil {
